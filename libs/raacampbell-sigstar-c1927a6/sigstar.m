function varargout=sigstar(groups,stats,nosort,showAll,lineColor, ns, lineHeight)
    ...
    y=ylim;
        ...
        yd=myRange(y)*0.125; %separate sig bars vertically by 5% 
        
    for ii=1:length(groups)
        if ii > 1 && ~isempty(lineHeight)
            yd=myRange(y)*lineHeight(3);
        end
        thisY=findMinY(xlocs(ii,:))+yd;
        H(ii,:) = makeSignificanceBar(xlocs(ii,:),thisY,stats(ii), lineColor, lineHeight, ns);
    end
    %-----------------------------------------------------
    ...
    %for all bars.
    yd=myRange(ylim)*0.02; %Ticks are 1% of the y axis range
    for ii=1:length(groups)
        y=get(H(ii,1),'YData');
        y(1)=y(1)-yd;
        y(4)=y(4)-yd;   
...
function H=makeSignificanceBar(x,y,p,lineColor,lineHeight, showAll)
    % produces the bar and defines how many asterisks we get for a 
    %given p-value
    ...
    if p<=1E-3
        stars='***'; 
        pStr = [' p=' scientificNotationConverter(p)];
    elseif p<=1E-2
        stars='**';
        pStr = [' p=' scientificNotationConverter(p)];
    elseif p<=0.05
    ...
    H(1)=plot(x(:),y - myRange(ylim)*lineHeight(1),lineColor,'LineWidth',1.25,'Tag','sigstar_bar');
    ...
    starY=mean(y) + myRange(ylim)*lineHeight(2);
    H(2)=text(mean(x(:)),starY,pStrPlot,...
    ...
    Y=ylim;
    if Y(2)<starY
        ylim([Y(1),starY+myRange(Y)*0.025])
    end
